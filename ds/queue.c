...
struct queue* queue_create(size_t capacity)
{
    ...
    q->first = q->last = NULL;
    q->data_length = 0; q->capacity = capacity;
}
...
int queue_remove(struct queue* q, int* value)
{
    ...
    if(q->data_length > 0)
    {
        *value = *q->first; q->data_length--; q->first++;
...
int main()
{
    ...
    for(int i=0; i<20 ; i++)
    {
        for (int j=0; j<2; j++)
        {
            queue_insert(q, 2*i+j); printf("val:%d first:%d last:%d length:%d pfirst:%p plast:%p\n", 2*i+j, *q->first, *(q->last-1), q->data_length, q->first, q->last);
        }
        queue_remove(q, &top); printf("top:%d first:%p last%p\n", top, q->first, q->last);
    }

    // apparently pointless. All memory will be returned to the OS when process quits.
    queue_free(q);
}
*/
