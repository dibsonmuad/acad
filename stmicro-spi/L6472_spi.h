...
#define MAXRXLEN 3
/** The maximum number of daisy chained spi devices supported in this implementation */
...
/** The set of commands that can be sent to the ST Micro L647X class of motor controllers */
typedef enum {
    NOP=0,
    SET_PARAM,
    GET_PARAM,
 ...
 */
typedef struct {
    uint8_t address;    /** The address tells the controller the actual register that is read or written to. */
    REG_NAME name;      /** REG_NAME is described above and makes register addresses human readable. */
    uint8_t len;        /** Length of the register in bits. */
    REG_TYPE type;      /** REG_TYPE is described above, but it is currently not utilized */
} register_entry;
...
typedef struct {
    ...
    uint8_t mask;       /** Helps setup the actual command code expected by the motor controller. */
    const char* fmt_str;/** */
    uint8_t tx_len;
    uint8_t rx_len;
} command_entry;
...
/** This function can be directly invoked after populating a spidev_config struct or is invoked internally from spi_initialize. */
