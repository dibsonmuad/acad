...
void get_token()
{
    ...
    {
        switch (state)
        { /* assuming that the state is 1 while it starts*/
        case 1:
        ...
        case 6:
            ...
            if (gl_reading_stmts == 1)
            {
                ...
                {
                    fprintf(gl_error_log,
                            "Line:%d: undefined identifier \"%s\"\nUndeclared "
                            "identifiers will be reported only once\n",
                            gl_lineno + 1,
                            lexeme);
                    printf("Line:%d: undefined identifier \"%s\"\n",
                           ...
                           lexeme);
                    gl_nerrors++;
                    // gl_nexttoken = insert(lexeme,ID);//insert anyway because
                    // I don't
                    // want to change the parser in any way. Note that..I am
                    // incrementing nerrors. So the symboltable and quad table
                    // will not be printed to output files
                }
            }
        ...
        case 18:
            ...
            if (c == EOF)
            {
                fprintf(gl_error_log,
                        "%s",
                        "Reached End Of File..Unclosed comment\n");
                exit(0);
            }
            if (c == '*')
                state = 19;
