...
void parse_error()
{
    ...
    while (1)
    {
        if ((token_type == SCOLON) || (token_type == ELSE) || (token_type == FI)
            || (token_type == OD)
            || (token_type == PCNT))
            break;
        else
...
bool D1()
{
    ...
    else if (gl_nexttoken->type == PCNT)
    {
        return true;
    }
    else
...
bool D()
{
            ...
            if (gl_nexttoken->type == CONS)
            {
                ...
                if (D2())
                {
                    if (gl_nexttoken->type == RSB)
                    {
                        ...
                        // This reverses the array dimensions to their correct
                        // order
                        ...
                        {
                            // current dimension starting from first: like if we
                            // had  x y z , x first, y second, z third will be the
                            // "pop order"

                            p_arr->dim++;
                            p_arr->dims[index] =
                                atoi(symboltable[arr_dim]->name);
    ...
    {
        // Sem Action : pop the stack until empty and assign the data_type
        // INTEGER
        // to all the elements
        print_stack(&SAS);
        while (SAS.top != 0)
    ...
    case (ID):
        ...
        if (T1())
        {
            if (gl_nexttoken->type == ASSGN)
            {
                ...
                if (E())
                {
                    // Sem action : 1. when E returns I'll find 2 IDs on the
                    // stack.
                    t_op1 = pop(&SAS);
                    t_res = pop(&SAS);
                ...
                if (E())
                {
                    ...
                    if (gl_nexttoken->type == TO)
                    {
                        ...
                        if (E())
                        {
                            ...
                            push(&SAS, gl_nquads);
                            temp_tok = gen_temp();
                            /* to make my quads consistent, all my conditionals
                            ...
                            if (gl_nexttoken->type == DO)
                            {
                                ...
                                if (S())
                                {
                                    if (gl_nexttoken->type == OD)
                                    {
                                        // Sem actions
                                        t_op1 = pop(
                                            &SAS); // contains quad no. of JEQ
                                        t_op2 = pop(
                                            &SAS); // contains quad no. of GT ..
                                        // This is to make sure that '1' is in
                                        // the  symbol table
        ...
        if (E())
        {
            if (E1())
            {
                if (gl_nexttoken->type == RSB)
                {
                    ...
                    // dont have to do this because temp_tok has the absolute
                    // address of
