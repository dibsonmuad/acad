...
char* gen_readable_quad(quad* pq){
  ...
  switch(pq->q_type){
  case(Q_JMP): strncpy(str, "jmp", 3);
    break;
  case(Q_JEQ): strncpy(str, "jeq", 3);
    break;
  case(Q_READ): strncpy(str, "read", 4);
    break;
  case(Q_WRITE): strncpy(str, "write", 5);
    break;
  case(Q_READLN): strncpy(str, "readln", 6);
    break;
  case(Q_WRITELN): strncpy(str, "writeln", 7);
    break;
  case(Q_PLUS): strcpy(str, "+");
    break;
  case(Q_MINUS): strncpy(str,  "-", 1);
    break;
  case(Q_MUL): strncpy(str, "*", 1);
    break;
  case(Q_DIV):
    strncpy(str, "/", 1);
    break;
  ...
  sprintf(final_str,"|%s|%s|%s|%s|%s|%s",q_no_str,q_str,op1_str,op2_str,res_str,inst_no_str);
  return final_str;
}
