...
spidev_config config_Spidev = { .spi_bus = "/dev/spidev1.0", .speed=1000000, .delay=0, .bits=8, .spi_mode=3, .ndevices=0 };
...
#if DEBUG
        fprintf(logfile,"byte %d: %d\n", i, (value >> ((tx_len-1 - i) * 8) & 0xFF));
#endif
...
//Receive Buf
    if (rx_len>0)
    {
        for (uint8_t i=tx_len; i< rx_len + tx_len; i++)
        {
            ...
            spi_transfer[i].rx_buf = (unsigned long) rx_buf[i-tx_len]; //( rx_buf + (i - tx_len) );
#if DEBUG_TRANSFER
            fprintf(logfile, "TRX:");
            for (uint8_t ret = 0; ret < spi_transfer[i].len; ret++)
            {
...
#if DEBUG
    fprintf(logfile, "device: %d, tx_len: %d, rx_len: %d\n", device, tx_len, rx_len);
#endif
...
#if (SPOOF_TRANSFER==0)
        ret = ioctl(fdspi, SPI_IOC_MESSAGE(ntransfers), \
                &spi_transfer);
        if (ret < 1) {
            pabort("can't send spi message");
        }
...
void set_param(uint8_t device, uint8_t reg, int value)
{
    ...
    tx_msg[0] = (uint8_t) ( (registers[reg].address & 0x1F) | commands[cmd_id].mask);
    //override txbytes - upto 4 bytes
    param_len = registers[reg].len;
    tx_len = ( !(param_len % 8) ? (param_len/8) : (param_len/8 + 1)) /*arg len*/ + 1 /*cmd len*/;
#if DEBUG
    fprintf(logfile,"param:%.2X value:%d, %0X%d\n", registers[reg].address, value, value);
#endif
...
int get_param(uint8_t device, uint8_t param)
{
    ...
    uint8_t tx_len = commands[cmd_id].tx_len, rx_len=0, param_len=registers[param].len;
    tx_msg[0] = (uint8_t) ((param & 0x1F) | commands[cmd_id].mask);
    //Recv
    rx_len = (!(param_len % 8) ? (param_len/8) : (param_len/8 + 1))  /*arg len only*/ ;
    transfer(device, tx_len, rx_len);
    if(_parallel_transfer)
        return 0;
...
{
    return exec_cmd(device, GO_HOME);
}

int go_mark(uint8_t device)
{
    return exec_cmd(device, GO_MARK);
}

int reset_pos(uint8_t device)
{
    return exec_cmd(device, RESET_POS);
}

int reset_device(uint8_t device)
{
    return exec_cmd(device, RESET_DEVICE);
}

int soft_stop(uint8_t device)
{
    return exec_cmd(device, SOFT_STOP);
}

int hard_stop(uint8_t device)
{
    return exec_cmd(device, HARD_STOP);
}

int soft_hiz(uint8_t device)
{
    return exec_cmd(device, SOFT_HIZ);
}

int hard_hiz(uint8_t device)
{
    return exec_cmd(device, HARD_HIZ);
}

int get_status(uint8_t device)
{
    return exec_cmd(device, GET_STATUS);
}
...
{
    ndevices = config_Spidev.ndevices = n;
}
